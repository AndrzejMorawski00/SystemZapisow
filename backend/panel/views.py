...
from panel.data_types import CourseDataDict
from panel.utils import fetch_semester_data, has_metadata, has_permission, process_courses, schema

from panel.site_crawler import SiteCrawler
from panel.forms import CourseForm, SemesterForm
from panel.decoratos import user_authenticated
from panel.models import Semester, CourseEffect, CourseTag, CourseType, Course
...
def fetch_semesters_view(request: HttpRequest):
    ...
    sc = SiteCrawler()
    counter = 0
    for semester in sc.get_semesters():
        try:
            Semester.objects.get(link=semester['link'])
...
def semester_list_view(request: HttpRequest):
    if request.method == 'POST':
        ...
        counter = fetch_semester_data(selected_semesters)
        if counter == 0:
    ...
    return render(request, 'panel/semester_list_view.html',
                  {'semester_list': semester_list,
                   'column_names':
                   ['Semester', 'Fetched?',
                    'Select semesters to fetch data', 'View Subjects']})
...
def add_subject_list(request: HttpRequest):
    if request.method == 'POST':
        ...
        try:
            ...
            counter = process_courses(courses, semester, False)
...
def metadata_list_view(request: HttpRequest, datatype: Literal['tag', 'effect', 'type']):
    ...
    if datatype in datatype_dict:
        ...
        paginator = Paginator(data, MAX_PAGE_SIZE)
        page_obj = paginator.get_page(page_number)
        return render(request, 'panel/metadata_list_view.html',
                      {'page_obj': page_obj,
                       'name': name,
                       'datatype': datatype,
                       'edit': edit})
...
def metadata_edit_view(request: HttpRequest, datatype: Literal['tag', 'effect', 'type'], obj_pk: int):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'panel/metadata_edit_view.html',
                      {'model': obj, 'name': name, 'datatype': datatype})
        ...
        if request.method == 'POST':
            ...
            if course_form.is_valid():
                ...
                return render(request,
                              'panel/subject_edit_view.html',
                              {'course_form': course_form, 'semester_id': semester_id})
        else:
            course_form = CourseForm(instance=course)
            return render(request, 'panel/subject_edit_view.html',
                          {'course_form': course_form, 'semester_id': semester_id})
...
def handle_semester_view(request: HttpRequest,  obj_pk: Optional[int] = None):
    if request.method == "POST":
        ...
        if semester_form.is_valid():
            custom_pk = semester_form.cleaned_data.get('custom_pk')
            if custom_pk is not None:
                semester = semester_form.save(commit=False)
                semester.pk = custom_pk
                semester.save()
            else:
                semester_form.save()
            messages.success(request, "Changes saved")
            ...
            messages.error(request, "Something went wrong")
            return render(request, 'panel/handle_semester_view.html',
                          {'semester_form': semester_form, 'action': 'Edit' if obj_pk else 'Add'})
        return redirect('panel:semester-list-view')
